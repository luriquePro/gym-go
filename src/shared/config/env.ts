...
const envSchema = z
  .object({
    // Environment
    NODE_ENV: z
      .enum(Object.values(NODE_ENV), "NODE_ENV deve ser um dos seguintes valores: " + Object.values(NODE_ENV).join(", "))
      .default(DEFAULT_VALUES.NODE_ENV),
    APP_AMBIENT: z
      .enum(Object.values(APP_AMBIENT), "APP_AMBIENT deve ser um dos seguintes valores: " + Object.values(APP_AMBIENT).join(", "))
      .default(DEFAULT_VALUES.APP_AMBIENT),
    PORT: z.coerce
      .number("PORT deve ser um número")
      .min(ENV_VALIDATIONS.PORT.MIN, `PORT deve ser maior que ${ENV_VALIDATIONS.PORT.MIN}`)
      .max(ENV_VALIDATIONS.PORT.MAX, `PORT deve ser menor que ${ENV_VALIDATIONS.PORT.MAX}`)
    ...
    DB_URI: z
      ...
      .or(z.url().startsWith("mongodb+srv://", "DB_URI deve ser uma URL Válida")),
    ...
    // Security
    JWT_SECRET: z.string("JWT_SECRET deve ser uma string").min(32, "JWT_SECRET deve ter pelo menos 32 caracteres"),
    JWT_EXPIRES_IN: z.string("JWT_EXPIRES_IN deve ser uma string").default(DEFAULT_VALUES.JWT_EXPIRES_IN),
    BCRYPT_ROUNDS: z.coerce
      .number("BCRYPT_ROUNDS deve ser um número")
      .min(ENV_VALIDATIONS.BCRYPT_ROUNDS.MIN, `BCRYPT_ROUNDS deve ser maior que ${ENV_VALIDATIONS.BCRYPT_ROUNDS.MIN}`)
      .max(ENV_VALIDATIONS.BCRYPT_ROUNDS.MAX, `BCRYPT_ROUNDS deve ser menor que ${ENV_VALIDATIONS.BCRYPT_ROUNDS.MAX}`)
      .default(DEFAULT_VALUES.BCRYPT_ROUNDS),

    // Rate Limiting
    RATE_LIMIT_MAX: z.coerce
      .number("RATE_LIMIT_MAX deve ser um número")
      .min(ENV_VALIDATIONS.RATE_LIMIT_MAX.MIN, `RATE_LIMIT_MAX deve ser maior que ${ENV_VALIDATIONS.RATE_LIMIT_MAX.MIN}`)
      .max(ENV_VALIDATIONS.RATE_LIMIT_MAX.MAX, `RATE_LIMIT_MAX deve ser menor que ${ENV_VALIDATIONS.RATE_LIMIT_MAX.MAX}`)
    ...
    CORS_CREDENTIALS: z.coerce.boolean("CORS_CREDENTIALS deve ser um booleano").default(DEFAULT_VALUES.CORS_CREDENTIALS)
  })
  .superRefine((data, { addIssue }) => {
    const nodeEnv = data.NODE_ENV;
    const appAmbient = data.APP_AMBIENT;

    if (nodeEnv === NODE_ENV.PRODUCTION || appAmbient === APP_AMBIENT.PRODUCTION) {
    ...
    if (nodeEnv === NODE_ENV.DEVELOPMENT || appAmbient === APP_AMBIENT.DEVELOPMENT) {
